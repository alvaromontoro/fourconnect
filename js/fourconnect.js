class FourConnect extends HTMLElement {
	...
	showResults() {
		for (let x = 0; x < 4; x++) {
			...
			p.textContent = [...this.board.querySelectorAll(`:scope > [data-d="${x}"]`)]
		...
		this.shadow
			.querySelectorAll(".scrim")
			.forEach((el) => el.classList.remove("visible"));
	}

	initalizeValues() {
    this.colors = ["#fe5", "#fc6", "#ffa33d", "#ff744b"];
		this.errors = [];
		this.groups = [];
		this.buttons = [];
		if (this.board) {
	...
	generateBoard() {
		...
		for (let x = 0; x < 4; x++) {
			for (let y = 0; y < 4; y++) {
				...
				button.dataset.d = x;
				button.innerHTML = data[x].values[y];
	...
	connectedCallback() {
		...
		this.shadow
			.querySelector(".scrim-good button")
			.addEventListener("click", this.showResults.bind(this));
		this.shadow
			.querySelector(".scrim-bad button")
			.addEventListener("click", this.showResults.bind(this));
...
.connect-4 {
  ...
  @media (min-width: 768px) {
    --size: 500px;
    --size: calc(var(--container-max-width) - 2rem);
    width: var(--size);
  }
